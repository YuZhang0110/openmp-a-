...
public:
    ...
    void exploreNeighbors(const AS_Point& cur, int old_health, int cur_cost) {
		if (cur.x - 1 >= 0){
	...
	bool isDestinationReached(const AS_Point& cur) {
...
#pragma omp parallel
		{
			while (1){
				...
				exploreNeighbors(cur, old_health, cur_cost);
